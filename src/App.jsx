...
export default function App() {
  ...
  const recategorizarTudo=async()=>{if(!hasAI)return;setRecatLoading(true);const filtered=txs.filter(t=>(recatFilter.conta==="all"||t.conta===recatFilter.conta)&&(recatFilter.src==="all"||t.src===recatFilter.src)&&(recatFilter.cat==="all"||t.cat===recatFilter.cat));const chunks=[];for(let i=0;i<filtered.length;i+=50)chunks.push(filtered.slice(i,i+50));for(const chunk of chunks){const result=await categorizarComIA(chunk,catRules);for(const tx of result){await supabase.from("transactions").update({cat:tx.cat,type:INCOME_CATS.includes(tx.cat)?"in":"out"}).eq("id",tx.id);}}const{data}=await supabase.from("transactions").select("*").order("date",{ascending:false});if(data)setTxs(data);setRecatLoading(false);alert("Concluído: "+filtered.length+" transações recategorizadas.");};
