...
function AccountsPage({accounts,setAccounts,txs,setTxs,saveTx}) {
  ...
  const faturasPendentes=accounts.filter(a=>a.tipo==="credito"&&a.ativo).flatMap(acc=>{
    ...
    const g={};pend.forEach(t=>{const m=t.fatura_mes||t.date?.slice(0,7)||"";if(!g[m])g[m]={mes:m,total:0,ids:[]};g[m].total+=Math.abs(Number(t.value));g[m].ids.push(t.id);});
      ...
      {page==="home"&&<>
        ...
        {!loadingTxs&&<>
          ...
          {catData.length>0&&<div style={card}>
            ...
            {editGoals&&<div style={{marginTop:12,borderTop:"1px solid "+T.border,paddingTop:12}}><div style={{fontSize:12,fontWeight:700,color:T.sub,marginBottom:10}}>META MENSAL (R$)</div>{EXPENSE_CATS.map(c=>(<div key={c} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:13,flex:1}}>{CATS[c].icon} {c}</span><input type="number" defaultValue={goals[c]||""} placeholder="0" onChange={e=>setGoalDraft(g=>({...g,[c]:parseFloat(e.target.value)||0}))} style={{width:90,padding:"6px 10px",border:"1.5px solid "+T.border,borderRadius:8,fontFamily:M,fontSize:13,outline:"none",textAlign:"right"}}/></div>))}<button onClick={saveGoals} style={{width:"100%",marginTop:8,padding:"11px",background:T.accent,color:"#fff",border:"none",borderRadius:10,fontFamily:F,fontSize:14,fontWeight:700,cursor:"pointer"}}>Salvar metas</button></div>}
          </div>}
