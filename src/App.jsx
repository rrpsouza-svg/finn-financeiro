...
async function aiCall(messages,system,maxTokens=800) {
  ...
  const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":KEY,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:maxTokens,system,messages})});
    ...
    <div style={{padding:"14px 14px 0"}}>
      ...
      {page==="home"&&<>
        ...
        {!loadingTxs&&<>
          ...
          <div style={card}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><span style={{fontWeight:700,fontSize:14}}>Últimas movimentações</span><div style={{display:"flex",gap:12}}><button onClick={()=>setPage("comparar")} style={{background:"none",border:"none",color:T.accent,fontSize:12,fontWeight:700,cursor:"pointer",padding:0}}>🔍 Comparar</button><button onClick={()=>setPage("extrato")} style={{background:"none",border:"none",color:T.accent,fontSize:12,fontWeight:700,cursor:"pointer",padding:0}}>Ver extrato</button></div></div>
      ...
      {page==="chat"&&<>
        <div style={{background:T.dark,borderRadius:"16px 16px 0 0",padding:"13px 16px",display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1}}><div style={{color:"#fff",fontWeight:700,fontSize:14}}>Finn {!hasAI&&<span style={{fontSize:11,color:T.yellow}}>inativa</span>}</div><div style={{color:"#8b93b0",fontSize:11,display:"flex",alignItems:"center",gap:4}}><div style={{width:6,height:6,borderRadius:99,background:hasAI?T.green:T.yellow}}/>{hasAI?"online":"sem chave"} - {filteredTxs.length} transações - {monthLabel}</div></div>
          <button onClick={()=>{if(window.confirm("Limpar a conversa atual? O histórico salvo não será apagado, mas a Finn vai esquecer o contexto desta sessão.")){setChatLog([{role:"assistant",content:"Conversa reiniciada! Sou a Finn, tenho acesso direto aos seus dados financeiros. O que você quer saber ou fazer?"}]);}}} style={{background:"rgba(255,255,255,.1)",border:"none",borderRadius:8,padding:"6px 10px",color:"#fff",fontSize:11,fontWeight:600,cursor:"pointer",flexShrink:0}}>🗑️ Limpar</button>
        </div>
