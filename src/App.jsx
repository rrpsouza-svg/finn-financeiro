...
// Default monthly goals per category (R$)
const DEFAULT_GOALS = {
...
function LoginScreen() {
  ...
  const handleForgot=async()=>{
    ...
    setMsg("✅ E-mail de recuperação enviado!");setLoading(false);
  };
  return (
...
export default function App() {
  ...
  const catData = EXPENSE_CATS.map(c=>({
    label:c,...CATS[c],val:filteredTxs.filter(t=>t.cat===c&&t.type==="out").reduce((a,t)=>a+Math.abs(Number(t.value)),0)
  })).filter(d=>d.val>0).sort((a,b)=>b.val-a.val);

  const incomeData = INCOME_CATS.map(c=>({
    label:c,...CATS[c],val:filteredTxs.filter(t=>t.cat===c&&t.type==="in").reduce((a,t)=>a+Number(t.value),0)
  })).filter(d=>d.val>0);
  ...
  const updateTx=async(tx)=>{
    const{error}=await supabase.from("transactions").update({descricao:tx.descricao,value:tx.value,cat:tx.cat,type:tx.type,date:tx.date}).eq("id",tx.id);
    setEditTx(null);
    if(!error){const{data}=await supabase.from("transactions").select("*").order("date",{ascending:false});if(data)setTxs(data);}
  };
  ...
  const recategorizarTudo=async()=>{
    ...
    const chunks=[];
    for(let i=0;i<txs.length;i+=50)chunks.push(txs.slice(i,i+50));
    let updated=0;
    for(const chunk of chunks){
      const result=await categorizarComIA(chunk);
      for(const tx of result){
        await supabase.from("transactions").update({cat:tx.cat,type:INCOME_CATS.includes(tx.cat)?"in":"out"}).eq("id",tx.id);
        updated++;
      }
    }
    ...
    alert(`✅ ${updated} transações recategorizadas!`);
  };

  // ── Monthly summary ──
  const gerarResumo=async()=>{
    if(!hasAI)return;
    setResumoLoading(true);setResumo(null);
    const [y,m]=selMonth.split("-").map(Number);
    ...
    const reply=await aiCall(
      [{role:"user",content:`Gere um resumo financeiro mensal de ${label} para um casal brasileiro. Seja objetivo, use emojis, máximo 200 palavras.\n\nDados:\nReceitas: R$${income.toFixed(2)}\nDespesas: R$${expense.toFixed(2)}\nSaldo: R$${balance.toFixed(2)}\n\nTransações:\n${lista}`}],
      "Você é um consultor financeiro pessoal. Gere resumos claros e úteis em português."
    );
...
Para registrar transação, inclua no final: <<<{"descricao":"...","value":0,"cat":"...","type":"in|out","date":"YYYY-MM-DD"}>>>`;
    try{
      const reply=await aiCall(history.map(m=>({role:m.role,content:m.content})),system)||"⚠️ Finn IA não está ativa. Adicione REACT_APP_ANTHROPIC_KEY no Vercel.";
      let r=reply;
      const jm=r.match(/<<<({.*?})>>>/s);
  ...
  const saveGoals=()=>{
    const merged={...goals,...goalDraft};
    setGoals(merged);
    try{localStorage.setItem("finn_goals",JSON.stringify(merged));}catch{}
    setEditGoals(false);setGoalDraft({});
  };
      ...
      <div style={{padding:"14px 14px 0"}}>
        ...
        {page==="home"&&<>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,overflowX:"auto",paddingBottom:2}}>
            ...
            {availableMonths.map(m=>{
              ...
              return <button key={m} onClick={()=>setSelMonth(m)} style={{padding:"6px 14px",borderRadius:99,border:`1.5px solid ${selMonth===m?T.accent:T.border}`,background:selMonth===m?T.accentLt:"transparent",color:selMonth===m?T.accent:T.sub,fontFamily:F,fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>{MONTHS_PT[mo-1].slice(0,3)} {y}</button>;
            })}
          </div>
          ...
          {!loadingTxs&&<>
            ...
            <div style={{...card,padding:14,marginBottom:12}}>
              ...
              {expenseDiff!==null&&selMonth!=="all"&&(
                <div style={{fontSize:12,color:T.sub,display:"flex",alignItems:"center",gap:6}}>
                  ...
                  <span style={{fontWeight:700,color:expenseDiff<=0?T.green:T.red}}>
                    {expenseDiff>0?"+":""}{expenseDiff.toFixed(1)}% em despesas {expenseDiff<=0?"✅":"⚠️"}
                  </span>
                </div>
              )}
            </div>
            ...
            {catData.length>0&&<div style={card}>
                      ...
                      {goals[d.label]&&(
                        <div style={{marginLeft:16}}>
                          <div style={{height:4,background:T.border,borderRadius:99}}>
                            <div style={{height:"100%",width:`${Math.min(100,(d.val/goals[d.label])*100)}%`,background:d.val>goals[d.label]?T.red:d.val>goals[d.label]*0.8?T.yellow:T.green,borderRadius:99,transition:"width .6s"}}/>
                          </div>
                          <div style={{fontSize:10,color:d.val>goals[d.label]?T.red:T.sub,marginTop:2}}>
                            {d.val>goals[d.label]?`⚠️ R$${(d.val-goals[d.label]).toFixed(0)} acima da meta`:`R$${(goals[d.label]-d.val).toFixed(0)} disponível`}
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <button onClick={()=>{setEditGoals(!editGoals);setGoalDraft({});}} style={{marginTop:10,background:"none",border:`1px solid ${T.border}`,borderRadius:8,padding:"6px 12px",fontSize:12,color:T.sub,cursor:"pointer",fontFamily:F,fontWeight:600}}>
                {editGoals?"Fechar metas":"⚙️ Editar metas"}
              </button>
            ...
            {selMonth!=="all"&&<div style={card}>
              ...
              {!hasAI&&<div style={{fontSize:12,color:T.sub,marginTop:6}}>Ative a Finn IA para gerar resumos automáticos.</div>}
        ...
        {page==="import"&&<>
          ...
          <div style={{...card,background:hasAI?T.accentLt:"#f8f8f8",border:`1px solid ${hasAI?T.accent+"33":T.border}`}}>
            <div style={{fontWeight:700,fontSize:13,marginBottom:6,color:hasAI?T.accent:T.sub}}>🤖 Recategorizar transações existentes</div>
            <div style={{fontSize:12,color:T.sub,marginBottom:12}}>A IA vai reclassificar todas as {txs.length} transações automaticamente.</div>
            <button onClick={recategorizarTudo} disabled={recatLoading||!hasAI} style={{width:"100%",padding:"12px",background:hasAI?T.accent:"#ccc",color:"#fff",border:"none",borderRadius:10,fontFamily:F,fontSize:14,fontWeight:700,cursor:hasAI?"pointer":"default",opacity:recatLoading?0.7:1}}>
              {!hasAI?"⚠️ Requer chave Anthropic":recatLoading?`Recategorizando ${txs.length} transações...`:"Recategorizar tudo com IA"}
            </button>
          </div>
        ...
        {page==="chat"&&<>
          ...
          <div style={{background:"#e5ddd5",padding:"14px 12px",display:"flex",flexDirection:"column",gap:4,height:340,overflowY:"auto",borderLeft:`1px solid ${T.border}`,borderRight:`1px solid ${T.border}`}}>
            {!hasAI&&<div style={{background:T.yellowLt,border:`1px solid ${T.yellow}33`,borderRadius:12,padding:"10px 14px",fontSize:12,color:"#8a6a00"}}>
              💡 Adicione <strong>REACT_APP_ANTHROPIC_KEY</strong> no Vercel para ativar a Finn IA.
            </div>}
