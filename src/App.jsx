...
function parseCSV(text) {
  ...
  return lines.slice(1).map(line=>{
    ...
    if (isNubank) {
      ...
      return {date:c[di]||new Date().toISOString().slice(0,10), descricao:c[ni]||"Transação", cat:"Outros", value:-raw, type:"out", src:"CSV", conta:"", status:"pendente",
        parcela_atual:inst?.atual||null, total_parcelas:inst?.total||null, grupo_parcela:inst?c[ni].replace(/\s*[Pp]arcela.*/,"").trim():""};
    }
...
function parseC6(text) {
  ...
  const C6_CATS = {
    ...
    "Educação":"Educação","Escola":"Educação","Lazer":"Lazer","Entretenimento":"Lazer","Cinema":"Lazer",
    "Vestuário":"Roupas","Roupa":"Roupas","Serviços":"Assinaturas","Assinatura":"Assinaturas","Streaming":"Assinaturas",
  };
  ...
  return lines.slice(1).map(line=>{
    ...
    let parcela_atual=null, total_parcelas=null, grupo_parcela="";
    if (parcela && parcela!=="Única") {
      const pm = parcela.match(/(\d+)\/(\d+)/);
      if (pm) { parcela_atual=parseInt(pm[1]); total_parcelas=parseInt(pm[2]); grupo_parcela=desc; }
    }
    const fullDesc = parcela&&parcela!=="Única"?desc+" - Parcela "+parcela:desc;
    return {date:parseDate(c[0]), data_compra:parseDate(c[0]), descricao:fullDesc, cat:mapCat(c[3],c[4]), value:-valor, type:"out", src:"CSV", conta:"", status:"pendente", parcela_atual, total_parcelas, grupo_parcela};
  }).filter(Boolean);
}
...
function parseModeloFinn(text) {
  ...
  return lines.slice(1).map(line=>{
    ...
    return {date:parseDate(c[iDataPag]), data_compra:parseDate(c[iDataComp]), descricao:c[iDesc], cat, value:isRec?valor:-valor, type:isRec?"in":"out", src:"modelo", conta:c[iConta]||"", status:"efetivado"};
  }).filter(Boolean);
}
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages",{
    ...
    body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2000,
      messages:[{role:"user",content:[
        ...
        {type:"text",text:`Extraia TODAS as transações de despesa deste extrato de cartão de crédito brasileiro.
Ignore: pagamentos de fatura, saldo anterior, totais, juros, encargos.
Para cada transação, identifique se é parcelada e qual parcela é.

Retorne APENAS JSON array sem markdown:
[{"date":"YYYY-MM-DD","descricao":"nome do estabelecimento","value":0.00,"cat":"categoria","parcela_atual":1,"total_parcelas":4,"grupo_parcela":"nome base sem parcela"}]

Categorias válidas: ${CAT_LIST.join(", ")}
- date: data da compra no formato YYYY-MM-DD (converta de DD/MM para YYYY-MM-DD)
- value: valor positivo em reais
- parcela_atual: número da parcela atual (null se à vista)
- total_parcelas: total de parcelas (null se à vista)
- grupo_parcela: nome da compra sem a info de parcela (null se à vista)`}
      ]}]
    })
  });
  ...
  try {
    ...
    return arr.map(t=>({
      date:t.date, data_compra:t.date, descricao:t.descricao, cat:t.cat||"Outros",
      value:-Math.abs(t.value), type:"out", src:"PDF", conta:"", status:"pendente",
      parcela_atual:t.parcela_atual||null, total_parcelas:t.total_parcelas||null, grupo_parcela:t.grupo_parcela||""
    }));
  } catch { return []; }
}
...
function gerarParcelasFuturas(parsed, existingTxs, faturaMes) {
  const futuras = [];
  const withInstallments = parsed.filter(t=>t.parcela_atual&&t.total_parcelas&&t.parcela_atual<t.total_parcelas);
  
  for (const tx of withInstallments) {
    const restantes = tx.total_parcelas - tx.parcela_atual;
    for (let i=1; i<=restantes; i++) {
      const mesFutura = addMonthsToYearMonth(faturaMes, i);
      const descFutura = tx.grupo_parcela
        ? tx.grupo_parcela+" - Parcela "+(tx.parcela_atual+i)+" de "+tx.total_parcelas
        : tx.descricao.replace(/[Pp]arcela\s+\d+\s+de\s+\d+/, "Parcela "+(tx.parcela_atual+i)+" de "+tx.total_parcelas)
                      .replace(/\b\d+\/\d+\b/, (tx.parcela_atual+i)+"/"+tx.total_parcelas);
      // Check if already exists
      const jaExiste = existingTxs.some(ex=>{
        const sameGrupo = tx.grupo_parcela && ex.grupo_parcela===tx.grupo_parcela;
        const sameParcela = ex.parcela_atual===(tx.parcela_atual+i) && ex.total_parcelas===tx.total_parcelas;
        const sameVal = Math.abs(Math.abs(Number(ex.value))-Math.abs(Number(tx.value)))<0.02;
        const sameMes = ex.fatura_mes===mesFutura||ex.date?.startsWith(mesFutura);
        return (sameGrupo||sameParcela)&&sameVal&&sameMes;
      });
      if (!jaExiste) {
        futuras.push({
          ...tx,
          date: mesFutura+"-05",
          data_compra: tx.data_compra||tx.date,
          descricao: descFutura,
          parcela_atual: tx.parcela_atual+i,
          fatura_mes: mesFutura,
          status:"pendente",
          _isFutura: true,
          _mesFutura: mesFutura,
        });
      }
    }
  }
...
function exportToExcel(txs, selMonth) {
  const header = ["DATA_PAGAMENTO","DATA_COMPRA","DESCRICAO","VALOR","TIPO","CONTA","CATEGORIA","STATUS","PARCELA","TOTAL_PARCELAS","FONTE"];
  const rows = txs.map(t => [
    t.date||"", t.data_compra||t.date||"", t.descricao||"",
    Math.abs(Number(t.value)).toFixed(2).replace(".",","),
    t.type==="in"?"Receita":"Despesa", t.conta||"", t.cat||"",
    t.status||"efetivado", t.parcela_atual||"", t.total_parcelas||"", t.src||"manual",
  ]);
  ...
  const summary=[[]
    ,["RESUMO"],["Total Receitas","R$ "+totalRec.toLocaleString("pt-BR",{minimumFractionDigits:2})]
    ...
    ,["Período",selMonth==="all"?"Todos":selMonth],["Gerado em",new Date().toLocaleDateString("pt-BR")]
  ];
  const csv=[header,...rows,...summary].map(r=>r.map(v=>"\""+String(v).replace(/"/g,"\"\"")+"\""  ).join(";")).join("\n");
  const blob=new Blob(["\uFEFF"+csv],{type:"text/csv;charset=utf-8;"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a");a.href=url;a.download="finn-"+selMonth+".csv";a.click();URL.revokeObjectURL(url);
}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:24,width:"100%",maxWidth:430,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{marginBottom:12}}>
          ...
          <select style={{...inp,padding:"10px 14px"}} value={form.conta} onChange={e=>setForm(f=>({...f,conta:e.target.value}))}>
        ...
        <div style={{marginBottom:12}}>
          ...
          <select style={{...inp,padding:"10px 14px"}} value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))}>
...
function LoginScreen() {
  ...
  const [loading,setLoading]=useState(false);const [msg,setMsg]=useState(null);const [forgot,setForgot]=useState(false);
  ...
  const handleLogin=async()=>{if(!email||!pass)return;setLoading(true);setMsg(null);const{error}=await supabase.auth.signInWithPassword({email,password:pass});if(error)setMsg("E-mail ou senha incorretos.");setLoading(false);};
  const handleForgot=async()=>{if(!email)return;setLoading(true);await supabase.auth.resetPasswordForEmail(email);setMsg("Enviado!");setLoading(false);};
  return (
    <div style={{minHeight:"100vh",background:T.dark,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,fontFamily:F}}>
      <div style={{width:"100%",maxWidth:360}}>
        ...
        <div style={{background:T.surface,borderRadius:20,padding:24,boxShadow:"0 8px 40px rgba(0,0,0,.3)"}}>
          ...
          {!forgot&&<><label style={{fontSize:11,fontWeight:700,color:T.sub,display:"block",marginBottom:6,textTransform:"uppercase",letterSpacing:.6}}>Senha</label><input style={inp} type="password" placeholder="..." value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()}/></>}
          ...
          <button onClick={forgot?handleForgot:handleLogin} disabled={loading} style={{width:"100%",padding:"14px",background:T.accent,color:"#fff",border:"none",borderRadius:12,fontFamily:F,fontSize:15,fontWeight:700,cursor:"pointer",opacity:loading?0.7:1}}>{loading?"Aguarde...":forgot?"Enviar":"Entrar"}</button>
          <button onClick={()=>{setForgot(!forgot);setMsg(null);}} style={{width:"100%",marginTop:12,background:"none",border:"none",color:T.sub,fontSize:12,cursor:"pointer",fontFamily:F}}>{forgot?"Voltar para login":"Esqueci minha senha"}</button>
        </div>
...
function AccountsPage({accounts,setAccounts,txs,setTxs,saveTx}) {
  ...
  const faturasPendentes=accounts.filter(a=>a.tipo==="credito"&&a.ativo).flatMap(acc=>{
    ...
    const grupos={};
    pendentes.forEach(t=>{const mes=t.fatura_mes||t.date?.slice(0,7)||"";if(!grupos[mes])grupos[mes]={mes,total:0,ids:[]};grupos[mes].total+=Math.abs(Number(t.value));grupos[mes].ids.push(t.id);});
    return Object.values(grupos).map(g=>({...g,conta:acc}));
  });

  const saveAccount=async(acc)=>{
    setSaving(true);
    if(acc.id){const{data}=await supabase.from("accounts").update({nome:acc.nome,saldo_inicial:acc.saldo_inicial,limite:acc.limite,dia_vencimento:acc.dia_vencimento,dia_fechamento:acc.dia_fechamento,ativo:acc.ativo}).eq("id",acc.id).select().single();if(data)setAccounts(p=>p.map(a=>a.id===acc.id?data:a));}
    else{const{data}=await supabase.from("accounts").insert([acc]).select().single();if(data)setAccounts(p=>[...p,data]);}
    setEditAcc(null);setSaving(false);
  };

  const handlePagarFatura=async()=>{
    if(!contaDebito||!pagarFatura)return;setPagando(true);
    for(const id of pagarFatura.ids){await supabase.from("transactions").update({status:"efetivado",conta_debito:contaDebito}).eq("id",id);}
    const{data}=await supabase.from("transactions").select("*").order("date",{ascending:false});
    if(data)setTxs(data);setPagando(false);setPagarFatura(null);setContaDebito("");
  };
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div><div style={{fontWeight:800,fontSize:18}}>Contas e Cartões</div><div style={{fontSize:12,color:T.sub,marginTop:2}}>{accounts.filter(a=>a.ativo).length} contas ativas</div></div>
        <button onClick={()=>setEditAcc({nome:"",tipo:"CC",banco:"",saldo_inicial:0,limite:0,dia_vencimento:5,dia_fechamento:25,ativo:true})} style={{padding:"8px 14px",background:T.accent,color:"#fff",border:"none",borderRadius:10,fontFamily:F,fontSize:13,fontWeight:700,cursor:"pointer"}}>+ Nova</button>
      </div>

      {faturasPendentes.length>0&&<div style={{marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:700,color:T.sub,textTransform:"uppercase",letterSpacing:.8,marginBottom:8}}>⏳ Faturas pendentes</div>
        {faturasPendentes.map((f,i)=>{
          const[y,m]=f.mes.split("-").map(Number);const mesLabel=MONTHS_PT2[m-1]+" "+y;
          return(<div key={i} style={{background:T.yellowLt,borderRadius:14,padding:16,marginBottom:8,border:"1px solid #f5b54444"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div><div style={{fontSize:13,fontWeight:700,color:"#8a6a00"}}>{f.conta.nome}</div><div style={{fontSize:11,color:"#c2880a",marginTop:2}}>Fatura {mesLabel} · {f.ids.length} lançamentos</div></div>
              <div style={{textAlign:"right"}}>
                ...
                <button onClick={()=>{setPagarFatura(f);setContaDebito("");}} style={{marginTop:6,padding:"6px 12px",background:"#c2880a",color:"#fff",border:"none",borderRadius:8,fontFamily:F,fontSize:12,fontWeight:700,cursor:"pointer"}}>Pagar →</button>
              </div>
            </div>
          </div>);
        })}
      ...
      {["CC","credito","investimento"].map(tipo=>{
        const accs=accounts.filter(a=>a.tipo===tipo&&a.ativo);if(!accs.length)return null;
        return(<div key={tipo} style={{marginBottom:16}}>
          <div style={{fontSize:11,fontWeight:700,color:T.sub,textTransform:"uppercase",letterSpacing:.8,marginBottom:8}}>{TIPO_ICONS[tipo]} {tipo==="CC"?"Contas Correntes":tipo==="credito"?"Cartões de Crédito":"Investimentos"}</div>
          {accs.map(a=>(
            <div key={a.id} style={{background:T.card,borderRadius:14,padding:16,marginBottom:8,boxShadow:T.shadow,border:"1px solid "+T.border}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div><div style={{fontSize:14,fontWeight:700}}>{a.nome}</div><div style={{fontSize:11,color:T.sub,marginTop:2}}>{a.banco}</div></div>
              ...
              <div style={{display:"flex",gap:16,marginTop:12}}>
                <div><div style={{fontSize:10,color:T.sub,textTransform:"uppercase",letterSpacing:.5}}>Saldo inicial</div><div style={{fontSize:15,fontWeight:700,color:T.green,fontFamily:M}}>R${Number(a.saldo_inicial).toLocaleString("pt-BR",{minimumFractionDigits:2})}</div></div>
                {a.tipo==="credito"&&<><div><div style={{fontSize:10,color:T.sub,textTransform:"uppercase",letterSpacing:.5}}>Limite</div><div style={{fontSize:15,fontWeight:700,color:T.accent,fontFamily:M}}>R${Number(a.limite).toLocaleString("pt-BR",{minimumFractionDigits:2})}</div></div>
                <div><div style={{fontSize:10,color:T.sub,textTransform:"uppercase",letterSpacing:.5}}>Fechamento/Venc</div><div style={{fontSize:13,fontWeight:700}}>Dia {a.dia_fechamento}/{a.dia_vencimento}</div></div></>}
              </div>
            </div>
          ))}
        </div>);
      ...
      {pagarFatura&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
        <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:24,width:"100%",maxWidth:430}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><span style={{fontSize:16,fontWeight:800}}>Pagar Fatura</span><button onClick={()=>setPagarFatura(null)} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:T.sub}}>x</button></div>
          <div style={{background:T.yellowLt,borderRadius:10,padding:"12px 14px",marginBottom:20}}>
            ...
            <div style={{fontSize:12,color:"#c2880a",marginTop:2}}>{pagarFatura.ids.length} lançamentos pendentes</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
            {accounts.filter(a=>a.tipo==="CC"&&a.ativo).map(a=>(
              <button key={a.id} onClick={()=>setContaDebito(a.nome)} style={{padding:"12px 16px",borderRadius:10,border:"2px solid "+(contaDebito===a.nome?T.accent:T.border),background:contaDebito===a.nome?T.accentLt:T.surface,color:contaDebito===a.nome?T.accent:T.dark,fontFamily:F,fontSize:14,fontWeight:600,cursor:"pointer",textAlign:"left"}}>🏦 {a.nome}</button>
            ))}
          </div>
          ...
          <div style={{fontSize:11,color:T.sub,textAlign:"center",marginTop:10}}>Efetiva todos os lançamentos pendentes dessa fatura</div>
        </div>
      </div>}

      {editAcc&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
        <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:24,width:"100%",maxWidth:430,maxHeight:"85vh",overflowY:"auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><span style={{fontSize:16,fontWeight:800}}>{editAcc.id?"Editar":"Nova"} Conta</span><button onClick={()=>setEditAcc(null)} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:T.sub}}>x</button></div>
          <label style={lbl}>Nome</label><input style={inp} value={editAcc.nome} onChange={e=>setEditAcc(a=>({...a,nome:e.target.value}))} placeholder="Ex: Nubank Crédito"/>
          <label style={lbl}>Banco</label><input style={inp} value={editAcc.banco} onChange={e=>setEditAcc(a=>({...a,banco:e.target.value}))} placeholder="Ex: Nubank"/>
          <label style={lbl}>Tipo</label>
          <select style={{...inp}} value={editAcc.tipo} onChange={e=>setEditAcc(a=>({...a,tipo:e.target.value}))}>
            <option value="CC">Conta Corrente</option><option value="credito">Cartão de Crédito</option><option value="investimento">Investimento</option>
          </select>
          <label style={lbl}>Saldo Inicial (R$)</label><input type="number" style={inp} value={editAcc.saldo_inicial} onChange={e=>setEditAcc(a=>({...a,saldo_inicial:parseFloat(e.target.value)||0}))}/>
          {editAcc.tipo==="credito"&&<>
            <label style={lbl}>Limite (R$)</label><input type="number" style={inp} value={editAcc.limite} onChange={e=>setEditAcc(a=>({...a,limite:parseFloat(e.target.value)||0}))}/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div><label style={lbl}>Dia Fechamento</label><input type="number" style={inp} value={editAcc.dia_fechamento} onChange={e=>setEditAcc(a=>({...a,dia_fechamento:parseInt(e.target.value)||25}))}/></div>
              <div><label style={lbl}>Dia Vencimento</label><input type="number" style={inp} value={editAcc.dia_vencimento} onChange={e=>setEditAcc(a=>({...a,dia_vencimento:parseInt(e.target.value)||5}))}/></div>
            </div>
          </>}
          <button onClick={()=>saveAccount(editAcc)} disabled={saving} style={{width:"100%",padding:"14px",background:T.accent,color:"#fff",border:"none",borderRadius:12,fontFamily:F,fontSize:15,fontWeight:700,cursor:"pointer",marginTop:8,opacity:saving?0.7:1}}>{saving?"Salvando...":"Salvar"}</button>
        </div>
      </div>}
    </div>
  );
...
export default function App() {
  ...
  const [importFiles,setImportFiles]=useState([]);const [importAccount,setImportAccount]=useState("");const [importFaturaMes,setImportFaturaMes]=useState("");
  ...
  const catData=EXPENSE_CATS.map(c=>({label:c,...CATS[c],val:filteredTxs.filter(t=>t.cat===c&&t.type==="out"&&t.status!=="pendente").reduce((a,t)=>a+Math.abs(Number(t.value)),0)})).filter(d=>d.val>0).sort((a,b)=>b.val-a.val);
  ...
  const handleFiles=useCallback(async(e)=>{
    ...
    setImportFiles(files);setImportAccount("");setImportFaturaMes("");setImportPreview([]);setImportFuturas([]);
    setImportStep("select_account");e.target.value="";
  },[]);
  ...
  const processImportFiles=useCallback(async(selectedAccount,faturaMes)=>{
    ...
    for(const file of importFiles){
      ...
      if(name.endsWith(".pdf")){
        ...
        else if(text.includes("Data de Compra;Nome no Cart")){parsed=parseC6(text);}
        else if(text.toUpperCase().includes("DATA_PAG")||text.toUpperCase().includes("GRUPO_PARCELA")){parsed=parseModeloFinn(text);}
    ...
    const preview=categorized.map(t=>{
      const dup=txs.find(ex=>{
        const sameVal=Math.abs(Math.abs(Number(ex.value))-Math.abs(Number(t.value)))<Math.abs(Number(t.value))*0.01;
    ...
    const effectiveFaturaMes=faturaMes||selMonth;
    let futuras=[];
    if(isCreditCard&&faturaMes){
      futuras=gerarParcelasFuturas(categorized,txs,faturaMes);
    }
    setImportPreview(preview);setImportFuturas(futuras.map(f=>({...f,selected:true})));
    setImporting(false);setImportStep("preview");
  },[importFiles,txs,hasAI,accounts,selMonth]);
  ...
  const confirmImport=useCallback(async()=>{
    ...
    const isCreditCard=selAcc?.tipo==="credito";
    const toInsert=toImport.map(({date,data_compra,descricao,cat,value,type,src,conta,status,parcela_atual,total_parcelas,grupo_parcela})=>({
      date:isCreditCard&&importFaturaMes?(importFaturaMes+"-05"):date,
      data_compra:isCreditCard?date:null,
      descricao,cat,value,type,src:src||"extrato",conta:conta||"",
      status:isCreditCard&&type==="out"?"pendente":(status||"efetivado"),
      fatura_mes:isCreditCard&&importFaturaMes?importFaturaMes:"",
      parcela_atual:parcela_atual||null,total_parcelas:total_parcelas||null,grupo_parcela:grupo_parcela||""
    }));
    const futurasInsert=futurasToImport.map(({date,data_compra,descricao,cat,value,type,src,conta,parcela_atual,total_parcelas,grupo_parcela,fatura_mes})=>({
      date,data_compra:data_compra||null,descricao,cat,value,type,src:src||"extrato",conta:conta||"",
      status:"pendente",fatura_mes:fatura_mes||"",
      parcela_atual:parcela_atual||null,total_parcelas:total_parcelas||null,grupo_parcela:grupo_parcela||""
    }));
    let inserted=0;
    const all=[...toInsert,...futurasInsert];
    ...
    setImportLog(p=>[inserted+" transações importadas ("+futurasInsert.length+" parcelas futuras projetadas)",...p]);
    setImporting(false);setImportStep("idle");setImportPreview([]);setImportFuturas([]);
  },[importPreview,importFuturas,importAccount,importFaturaMes,accounts]);
  ...
  const toggleMic=()=>{
    ...
    rec.onresult=(e)=>{
      ...
      const system="Você é a Finn. Hoje é "+today+". Responda em português.\nDados: Receitas R$"+income.toFixed(2)+" Despesas R$"+expense.toFixed(2)+" Saldo R$"+balance.toFixed(2)+"\nTransações:\n"+summary+"\nPara registrar: <<<{\"descricao\":\"...\",\"value\":0,\"cat\":\"...\",\"type\":\"in|out\",\"date\":\"YYYY-MM-DD\"}>>>";
      aiCall(history.map(m=>({role:m.role,content:m.content})),system).then(async reply=>{
        let r=reply||"IA não disponível.";
        const jm=r.match(/<<<({.*?})>>>/s);
        if(jm){try{const o=JSON.parse(jm[1]);const v=parseFloat(o.value)||0;await saveTx({date:o.date||new Date().toISOString().slice(0,10),descricao:o.descricao||"Transação",cat:o.cat||"Outros",value:o.type==="out"?-v:v,type:o.type||"out",src:"manual",conta:"",status:"efetivado",fatura_mes:"",parcela_atual:null,total_parcelas:null,grupo_parcela:""});r=r.replace(/<<<{.*?}>>>/s,"").trim();}catch{}}
  ...
  const shown=txs.filter(t=>{const srcOk=filterSrc==="all"||t.src===filterSrc;const contaOk=filterConta==="all"||t.conta===filterConta;return srcOk&&contaOk;});
  ...
  const monthOpts=Array.from({length:6},(_,i)=>{const d=new Date();d.setMonth(d.getMonth()+i);const y=d.getFullYear();const m=String(d.getMonth()+1).padStart(2,"0");return{val:y+"-"+m,label:MONTHS_PT[d.getMonth()]+" "+y};}).concat(Array.from({length:6},(_,i)=>{const d=new Date();d.setMonth(d.getMonth()-i-1);const y=d.getFullYear();const m=String(d.getMonth()+1).padStart(2,"0");return{val:y+"-"+m,label:MONTHS_PT[d.getMonth()]+" "+y};}));
      ...
      <div style={{padding:"14px 14px 0"}}>
        ...
        {page==="home"&&<>
          <div style={{display:"flex",gap:8,marginBottom:10,overflowX:"auto",paddingBottom:2}}>
            ...
            {availableMonths.slice(0,8).map(m=>{const[y,mo]=m.split("-").map(Number);return <button key={m} onClick={()=>setSelMonth(m)} style={{padding:"6px 14px",borderRadius:99,border:"1.5px solid "+(selMonth===m?T.accent:T.border),background:selMonth===m?T.accentLt:"transparent",color:selMonth===m?T.accent:T.sub,fontFamily:F,fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>{MONTHS_PT[mo-1].slice(0,3)} {y}</button>;})}
          </div>
          ...
          {!loadingTxs&&<>
            ...
            <div style={{...card,padding:14,marginBottom:12}}>
              ...
              {pendingExpense>0&&<div style={{marginTop:8,padding:"8px 10px",background:T.yellowLt,borderRadius:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:12,color:"#8a6a00"}}>⏳ Faturas a pagar</span>
                <span style={{fontSize:13,fontWeight:700,color:"#c2880a",fontFamily:M}}>R${pendingExpense.toLocaleString("pt-BR",{minimumFractionDigits:2})}</span>
              </div>}
            </div>

            <button onClick={()=>exportToExcel(filteredTxs,selMonth)} style={{width:"100%",marginBottom:12,padding:"12px",background:T.surface,border:"1.5px solid "+T.border,borderRadius:12,fontFamily:F,fontSize:13,fontWeight:700,color:T.dark,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📥 Exportar CSV — {monthLabel}</button>
            ...
            {catData.length>0&&<div style={card}>
              ...
              <button onClick={()=>{setEditGoals(!editGoals);setGoalDraft({});}} style={{marginTop:10,background:"none",border:"1px solid "+T.border,borderRadius:8,padding:"6px 12px",fontSize:12,color:T.sub,cursor:"pointer",fontFamily:F,fontWeight:600}}>{editGoals?"Fechar metas":"Editar metas"}</button>
              {editGoals&&<div style={{marginTop:12,borderTop:"1px solid "+T.border,paddingTop:12}}>
                ...
                {EXPENSE_CATS.map(c=>(
                  <div key={c} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:13,flex:1}}>{CATS[c].icon} {c}</span><input type="number" defaultValue={goals[c]||""} placeholder="0" onChange={e=>setGoalDraft(g=>({...g,[c]:parseFloat(e.target.value)||0}))} style={{width:90,padding:"6px 10px",border:"1.5px solid "+T.border,borderRadius:8,fontFamily:M,fontSize:13,outline:"none",textAlign:"right"}}/></div>
                ))}
            ...
            <div style={card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                ...
                <button onClick={()=>setPage("extrato")} style={{background:"none",border:"none",color:T.accent,fontSize:12,fontWeight:700,cursor:"pointer",padding:0}}>Ver extrato</button>
              </div>
              ...
              {filteredTxs.slice(0,6).map((t,i)=>(
                <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderTop:i>0?"1px solid "+T.border:"none"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:11,color:T.sub,marginTop:1}}>{t.date} - {t.cat}{t.conta?" - "+t.conta:""}{t.status==="pendente"?" · pendente":""}</div>
                  </div>
                  <span style={{fontSize:14,fontWeight:700,color:t.type==="in"?T.green:t.status==="pendente"?"#c2880a":T.red,fontFamily:M,flexShrink:0,opacity:t.status==="pendente"?0.7:1}}>{t.type==="in"?"+":"-"}R${Math.abs(Number(t.value)).toFixed(2)}</span>
                </div>
              ))}
            </div>
          </>}
        ...
        {page==="contas"&&<>
          <div style={{display:"flex",gap:8,marginBottom:16}}>
            ...
            <button onClick={()=>setPage("contas")} style={{flex:1,padding:"10px",borderRadius:10,border:"1.5px solid "+T.accent,background:T.accentLt,fontFamily:F,fontSize:13,fontWeight:700,cursor:"pointer",color:T.accent}}>🏦 Contas</button>
          </div>
        ...
        {page==="extrato"&&<>
          ...
          <div style={{display:"flex",gap:6,marginBottom:8,overflowX:"auto",paddingBottom:4}}>
            {["all","manual","OFX","CSV","PDF","modelo"].map(f=>(
          ...
          <div style={card}>
            ...
            {shown.map((t,i)=>(
              <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 0",borderTop:i>0?"1px solid "+T.border:"none",opacity:t.status==="pendente"?0.8:1}}>
                <div style={{width:36,height:36,borderRadius:12,background:(CATS[t.cat]?.color||T.accent)+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{CATS[t.cat]?.icon||"📦"}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.descricao}</div>
        ...
        {page==="import"&&<>
          ...
          {importStep==="select_account"&&<div style={card}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
              {accounts.filter(a=>a.ativo).map(a=>(
                <button key={a.id} onClick={()=>setImportAccount(a.nome)} style={{padding:"12px 16px",borderRadius:12,border:"2px solid "+(importAccount===a.nome?T.accent:T.border),background:importAccount===a.nome?T.accentLt:T.surface,color:importAccount===a.nome?T.accent:T.dark,fontFamily:F,fontSize:14,fontWeight:600,cursor:"pointer",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span>{a.nome}</span><span style={{fontSize:11,color:importAccount===a.nome?T.accent:T.sub}}>{a.banco}</span>
                </button>
              ))}
              <button onClick={()=>setImportAccount("")} style={{padding:"12px 16px",borderRadius:12,border:"2px solid "+T.border,background:T.surface,color:T.sub,fontFamily:F,fontSize:14,cursor:"pointer",textAlign:"left"}}>Não vincular a nenhuma conta</button>
            </div>
            {accounts.find(a=>a.nome===importAccount)?.tipo==="credito"&&<div style={{marginBottom:16}}>
              ...
              <div style={{fontSize:12,color:T.sub,marginBottom:8}}>Todas as transações serão agrupadas neste mês.</div>
          ...
          {importStep==="importing"&&<div style={{...card,textAlign:"center",padding:40}}>
            ...
            <div style={{fontSize:15,fontWeight:700,marginBottom:6}}>Processando...</div>
            <div style={{fontSize:13,color:T.sub}}>Categorizando, verificando duplicatas e projetando parcelas futuras</div>
          </div>}
          ...
          {importStep==="preview"&&<>
                ...
                <div key={i} onClick={()=>setImportPreview(prev=>prev.map((x,j)=>j===i?{...x,selected:!x.selected}:x))}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                      ...
                      {p.tx.total_parcelas&&<span style={{fontSize:9,padding:"2px 6px",borderRadius:99,background:"#f3e8ff",color:"#7c3aed",fontWeight:700,flexShrink:0}}>{p.tx.parcela_atual}/{p.tx.total_parcelas}</span>}
                    </div>
                    <div style={{fontSize:11,color:T.sub}}>{p.tx.date} · {p.tx.cat}{p.tx.conta?" · "+p.tx.conta:""}</div>
                    {p.isDuplicate&&p.duplicateOf&&<div style={{fontSize:11,color:"#c2880a",marginTop:2}}>Similar: {p.duplicateOf.descricao} ({p.duplicateOf.date})</div>}
                  </div>
            ...
            {importFuturas.length>0&&<div style={card}>
              ...
              <div style={{fontSize:12,color:T.sub,marginBottom:12}}>Serão lançadas como pendentes nos meses futuros para projeção de fluxo de caixa.</div>
              <div style={{display:"flex",gap:8,marginBottom:12}}>
                <button onClick={()=>setImportFuturas(f=>f.map(x=>({...x,selected:true})))} style={{flex:1,padding:"7px",background:"#f3e8ff",border:"1px solid #7c3aed44",borderRadius:8,color:"#7c3aed",fontFamily:F,fontSize:12,fontWeight:700,cursor:"pointer"}}>Selecionar todas</button>
              ...
              {importFuturas.map((f,i)=>(
                <div key={i} onClick={()=>setImportFuturas(prev=>prev.map((x,j)=>j===i?{...x,selected:!x.selected}:x))}
                  style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderTop:i>0?"1px solid "+T.border:"none",cursor:"pointer",opacity:f.selected?1:0.4}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:12,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.descricao}</div>
                    <div style={{fontSize:11,color:T.sub}}>{f._mesFutura} · {f.cat}</div>
                  </div>
            ...
            <div style={{display:"flex",gap:10,marginBottom:20}}>
              ...
              <button onClick={confirmImport} disabled={importing} style={{flex:2,padding:"13px",background:T.green,color:"#fff",border:"none",borderRadius:12,fontFamily:F,fontSize:15,fontWeight:700,cursor:"pointer",opacity:importing?0.7:1}}>
                {importing?"Importando...":"Importar "+(importPreview.filter(p=>p.selected).length+importFuturas.filter(f=>f.selected).length)+" transações"}
              </button>
            </div>
          </>}
        </>}
        ...
        {page==="chat"&&<>
          <div style={{background:T.dark,borderRadius:"16px 16px 0 0",padding:"13px 16px",display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              <div style={{color:"#8b93b0",fontSize:11,display:"flex",alignItems:"center",gap:4}}><div style={{width:6,height:6,borderRadius:99,background:hasAI?T.green:T.yellow}}/>{hasAI?"online":"sem chave API"} - {filteredTxs.length} transações - {monthLabel}</div>
            </div>
          </div>
